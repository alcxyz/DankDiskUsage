...
The Nix section runs `nix-store --query --requisites /run/current-system | wc -l` to count current generation paths and `nix path-info --closure-size --human-readable /run/current-system` for current generation closure size. While both are fast (~0.1s), the section showed `?` until the process completed because there was no cached data to display on initial load.
